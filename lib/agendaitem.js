...
export async function getAgendaitemDataFromReport(reportId) {
  const queryString = `
    ...
    SELECT ?agendaitem ?agendaitemId ?shortTitle ?title ?isApproval ?subcaseName ?agendaitemType ?decisionResultCode
    WHERE {
  ...
  return {
    ...
    isApproval: agendaitem.isApproval?.value === "1",
    subcaseName: agendaitem.subcaseName?.value,
    agendaitem: agendaitem.agendaitem.value,
    agendaitemId: agendaitem.agendaitemId.value,
    agendaitemType: agendaitem.agendaitemType.value,
    decisionResultCode: agendaitem.decisionResultCode?.value,
  };
}
...
export async function getAgendaitemPiecesForReport(agendaitemId, isApproval, isRetracted) {
  let queryString = `
    ${prefixHeaderLines.besluitvorming}
    ${prefixHeaderLines.dct}
