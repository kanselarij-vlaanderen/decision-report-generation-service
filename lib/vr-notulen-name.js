...
export default class VRNotulenName {
  ...
  static get regex() {
    ...
    return new RegExp(
      `^${regexGroup.context} PV ${regexGroup.year}/${regexGroup.sessionNr}(.*?)${regexGroup.versionSuffix}?$`
    );
  }
  ...
  vrNumberWithSuffix() {
    try {
      ...
      return `${meta.context} PV ${meta.year}/${meta.sessionNr}${
        meta.versionSuffix || ""
      }`;
    } catch (error) {
      return this.name;
...
export const compareFunction = function (parameterA, parameterB) {
  try {
    ...
    try {
      ...
      return (
        metaA.year - metaB.year || // Year ascending
        metaA.sessionNr - metaB.sessionNr || // Session number ascending
        metaA.versionNumber - metaB.versionNumber
